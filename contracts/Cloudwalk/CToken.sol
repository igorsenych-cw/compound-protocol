...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
     ...
     */
    function getTrustedAdmin(address account) override external view returns (bool) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setTrustedAdmin(address account, bool enabled) override external returns (uint) {
        if (msg.sender != admin) {
            revert SetTrustedAdminOwnerCheck();
        }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setTrustedSupplier(address account, bool exists, uint supplyAllowance) override external returns (uint) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setTrustedBorrower(address account, bool exists, uint borrowAllowance) override external returns (uint) {
        if (msg.sender != admin && !trustedAdmins[msg.sender]) {
     ...
     */
    function borrowTrustedInternal(uint totalAmount, uint borrowAmount, address treasury) internal nonReentrant {
        accrueInterest();
        ...
        if (borrowAmount > totalAmount) {
            revert TrustedBorrowAmountCheck();
        }

        // borrowFresh emits borrow-specific logs on errors, so we don't need to
        borrowFresh(payable(msg.sender), totalAmount);
